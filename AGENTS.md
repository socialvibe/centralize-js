...
2. Check the current branch (`git branch --show-current`):
 - **If on the default branch (`master`)**:
...
Never commit directly to `master` (company branch protection). Increment `package.json` in the PR (patch by default; minor/major when the change warrants it).
...
- CI fails the PR if `package.json` was not incremented vs `master`, or if `CHANGELOG.md` does not have exactly one matching `## vX.Y.Z` heading.
- After merge to `master`, CI publishes, tags, and creates the GitHub release. See [README.md](README.md) § Versioning & Releases.
