...
import centralizeDefault, { Hub, Logger, LoggerClass, createLogger, LOG_LEVELS, CreateMessage } from '../index.ts';
...
describe('createLogger', () => {
  it('creates a fully-typed logger that sends through the Hub', () => {
    const received: IMessage[] = [];
    const unsubscribe = Hub.messages.addReceiver((msg) => received.push(msg));

    const custom = createLogger(Hub, { critical: 100 });
    custom.critical('bad!');
