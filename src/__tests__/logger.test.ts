...
describe('constructor', () => {
  ...
  it('throws when a log level name is not alphanumeric', () => {
    const { sender } = createSender();

    // an underscore-prefixed name could otherwise collide with a private field like _sender
    assert.throws(() => new Logger<ILogLevels>(sender, { _sender: 1 }));
  });
});
